...
function buildSectionInstructions() {
  return [
    ...
    "2. company-snapshot / kind company-snapshot: Explain business structure. Use SEC filing metadata only if provided in ResearchEvidenceContext.",
    ...
    "5. financial-deep-dive / kind financial-deep-dive: If ResearchEvidenceContext has official-financial SEC facts, prioritize them. If missing, say the metric is missing and do not invent it.",
    "6. key-value-drivers / kind value-drivers: Explain what truly drives valuation and share price perception. Separate SEC facts, search draft items, and LLM analysis.",
    ...
    "9. catalysts / kind catalysts: Use recent-development or risk-catalyst facts from search evidence when supplied. Prefer high/medium confidence sources.",
    "10. key-risks / kind risks: Use search evidence or coverage missing items. Low-confidence sources are discussion signals only.",
...
function buildDataBoundaryInstructions(context?: ResearchEvidenceContext) {
  ...
  return [
    ...sourceNoteRules,
    "- sourceNote must say this has no real-time market price, no consensus estimates, no company IR narrative parsing, no database save, and no manual verification.",
    "- disclaimer.text must say: 本页面仅供研究和信息参考，不构成投资建议。",
...
function buildResearchEvidenceInstructions(context?: ResearchEvidenceContext) {
  ...
  return [
    ...
    "- Company Snapshot: may use filing-metadata and official-financial facts. Search evidence may only describe draft recent context.",
    ...
    "- Key Risks: use risk-catalyst facts or coverage missing items. Do not turn low-confidence sources into firm facts.",
    "- Variant Perception: may combine search themes with explicit caveats that SEC / IR / market data verification is still needed.",
    ...
    "- Monitoring Dashboard: include missing market price / consensus / IR follow-up metrics.",
